...
class NetpunktMockup {
  ...
  public function __construct() {
    $this->cache_folder = DRUPAL_ROOT . '/' . drupal_get_path('module', 'netpunkt_mockup') . '/cache';
    $this->log = variable_get('netpunkt_mockup_log', FALSE);
  }
  ...
  public function record($key, $data) {
    ...
    if (empty($data)) {
      ...
      throw new NetpunktMockupException('Failed to save mockup:' . $filename . ' is empty');
    }
    if ($f = fopen($filename, 'w')) {
      ...
      throw new NetpunktMockupException('Failed to save mockup:' . $filename . ' is not writeable');
    }
    return TRUE;
  }
  ...
  private function log($text) {
    ...
    $f = fopen(sys_get_temp_dir() . '/netpunkt_mockup_log.txt', 'a');
...
class NetpunktMockupException extends Exception {}
